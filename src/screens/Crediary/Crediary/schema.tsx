...
export default Yup.object().shape({
    ...
    localTrabalho: Yup.string().required('Por favor, informe local de trabalho'),
    estadoCivil: Yup.string().required('Por favor, selecione o estado civil'),
    nomeConjuge: Yup.string()
    .when("estadoCivil", (estadoCivil):any => {
      if(estadoCivil[0] == 'Casado') {
        return Yup.string().required("Por favor, informe o nome do conjuge")
      }
    }),
    cpfConjuge: Yup.string()
    .when("estadoCivil", (estadoCivil):any => {
      if(estadoCivil[0] == 'Casado') {
        return Yup.string().required("Por favor, informe o cpf do conjuge")
